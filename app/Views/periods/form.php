...
<body>
    ...
    <div class="content">
        ...
        <p style="color:#94a3b8;margin-top:-12px;"><?= htmlspecialchars($client['name'], ENT_QUOTES, 'UTF-8') ?></p>
                    ...
                    <table>
                        ...
                        <tbody>
                            <template x-for="row in rows" :key="row.id">
                                <tr>
                                    <td>
                                        <span :style="'display:inline-block;width:10px;height:10px;border-radius:50%;margin-right:6px;background:' + (row.color || '#3b82f6')"></span>
                                        <span x-text="row.name"></span>
                                        <input type="hidden" :name="'value_cents[' + row.id + ']'" :value="row.valueCents">
                                    </td>
                                    <td>
                                        <input type="text" inputmode="numeric" x-model="row.valueDisplay" @input="onValueInput(row)" style="width:140px;padding:8px 10px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0;">
                                    </td>
                                    <td>
                                        <input type="number" min="0" step="1" x-model.number="row.ordersCount" :name="'orders_count[' + row.id + ']'" style="width:100px;padding:8px 10px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0;">
                                    </td>
                                </tr>
                            </template>
                        </tbody>
