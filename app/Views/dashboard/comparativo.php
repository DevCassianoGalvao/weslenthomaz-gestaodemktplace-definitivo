...
foreach ($rows as $row) {
    if ($row['total_value_cents'] > 0) {
        ...
        $chartColors[] = $row['brand_color'] ?: '#3b82f6';
    }
}
?>
<!doctype html>
                    ...
                    <?php foreach ($rows as $row): ?>
                        <tr>
                            <td class="client-name">
                                <span style="display:inline-block;width:10px;height:10px;border-radius:50%;margin-right:6px;background:<?= htmlspecialchars($row['brand_color'] ?: '#3b82f6', ENT_QUOTES, 'UTF-8') ?>;"></span>
